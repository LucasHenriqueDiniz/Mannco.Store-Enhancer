...
const startProfileEnhancer = () => {
  ...
  chrome.storage.sync.get(Object.keys(config), (result) => {
    ...
    if (config.enableProfileEnhancer) {
      ...
      try {
        document.querySelector("#tab-dashboard > div:nth-child(1) > div:nth-child(2) > div > div > div.row > div:nth-child(1)").className = 'col-4'
        document.querySelector("#tab-dashboard > div:nth-child(1) > div:nth-child(2) > div > div > div.row > div.col-6.shelf-space").className = 'shelf-space col-4'
        } catch (err) {console.log(err)}
        
        const itemValue = document.querySelector("#tab-dashboard > div:nth-child(1) > div:nth-child(2) > div > div > div.row > div:nth-child(1) > span.important-text.ecurrency")
        ...
        document.querySelector("#tab-dashboard > div:nth-child(1) > div:nth-child(2) > div > div > div.row > div:nth-child(1)").insertAdjacentElement("afterend", afterFeesDiv);
        
        afterFeesValue = document.createElement('div')
        afterFeesValue.className = 'important-text'
        afterFeesValue.textContent = '$' + (itemValue - calculateFees(itemValue)).toFixed(2)
      ...
      function minimize(event) {
        ...
        const parentElement = button.parentNode.parentNode.parentNode.parentNode;
      ...
      // Create Buttons
      const BuyOrderCardHead = document.querySelector("#tab-history > div:nth-child(2) > div:nth-child(2) > div > div > div.card-head");

      // Transactions
      const DownloadTransaction = document.createElement('button');
      DownloadTransaction.className = 'btn btn-primary w-lg-100 float-right export';
      ...
      if (config.enableRemoveAllBuyOrderButton) {
        ...
        removeBuyOrders.className = 'btn btn-danger w-lg-100 float-right export';
      ...
      if (config.enableExportBuyOrderButton) {
        ...
        DowloadBuyOrder.className = 'btn btn-primary w-lg-100 float-right export';
